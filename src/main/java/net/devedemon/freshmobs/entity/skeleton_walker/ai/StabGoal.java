...
public class StabGoal extends MeleeAttackGoal {
    ...
    public StabGoal(PathfinderMob mob, double speedModifier, boolean followingTargetEvenIfNotSeen) {
    ...
    protected void checkAndPerformAttack(LivingEntity target, double distanceSquared) {
        ...
        double reach = getAttackReachSqr(target) + 1.5D;
    ...
    private void performAttackAnimation() {
        ...
        entity.playAttackAnimation("stab");
    }
    ...
    private boolean isEnemyWithinAttackDistance(LivingEntity target) {
        return mob.distanceToSqr(target) <= getAttackReachSqr(target) + 1.5D;
    }
